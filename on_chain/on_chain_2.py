...
class IntegratedBitcoinStrategy:
    ...
    def train_cnn_model(self, lookback_window=30):
        ...
        history = model.fit(
            ...
            epochs=10,  # Reduced for stability
            batch_size=64,
            verbose=1
        )
    ...
    def optimize_ensemble_weights(self, models, X_val, y_val, sum_to_1=True, 
        ...
        """
        Optimize ensemble weights using convex optimization or simple approach.
        ...
        if CVXPY_AVAILABLE:
            try:
                ...
                # Get optimal weights
                w_opt = w.value
                print("Optimized weights using CVXPY")
                
            except Exception as e:
                print(f"Error in CVXPY optimization: {str(e)}")
                print("Using simple R² based weights instead")
                w_opt = self._simple_weight_optimization(predictions, y_val)
        else:
    ...
    def _simple_weight_optimization(self, predictions, y_val):
        """
        Simple ensemble weight optimization based on R² scores.
        ...
        weights = np.array(r2_scores) / sum(r2_scores) if sum(r2_scores) > 0 else np.ones(M) / M
    ...
    def generate_trading_signals(self):
        ...
        if signal_ratio < 0.03:
            ...
            if 'vote_sum' in signals_df.columns:
                ...
                signals_df['signal'] = np.where(signals_df['vote_sum'] >= 1, 1, 
                                          np.where(signals_df['vote_sum'] <= -1, -1, 0))
    ...
    def _generate_regression_ensemble_signals(self, signals_df):
        """Generate signals from Regression Ensemble"""
    ...
    def _add_rule_based_signals(self, signals_df):
        """Add rule-based technical signals"""
        ...
        if 'nvt_ratio' in signals_df.columns:
            ...
            nvt_median = signals_df['nvt_ratio'].median()
            nvt_std = signals_df['nvt_ratio'].std()
            signals_df.loc[signals_df['nvt_ratio'] < nvt_median - 1.5 * nvt_std, 'nvt_signal'] = 1  # Undervalued - Buy
            signals_df.loc[signals_df['nvt_ratio'] > nvt_median + 1.5 * nvt_std, 'nvt_signal'] = -1  # Overvalued - Sell
        
        # HODL Ratio (Long-term vs Short-term holders)
        if 'hodl_ratio' in signals_df.columns:
            signals_df['hodl_signal'] = 0
            hodl_median = signals_df['hodl_ratio'].median()
            hodl_std = signals_df['hodl_ratio'].std()
            
            # More long-term holders = bullish signal (accumulated)
            signals_df.loc[signals_df['hodl_ratio'] > hodl_median + 1 * hodl_std, 'hodl_signal'] = 1
            
            # More short-term holders = bearish signal (distribution)
            signals_df.loc[signals_df['hodl_ratio'] < hodl_median - 1 * hodl_std, 'hodl_signal'] = -1
        ...
        if rule_based_cols:
            # Weight rule-based signals less than model signals
            signals_df['rule_based_signal'] = signals_df[rule_based_cols].sum(axis=1) * 0.5
    ...
    def _backtest_internal(self, initial_cash=100000):
        """Internal backtesting implementation"""
